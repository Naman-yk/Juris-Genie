...
const STORE_PATH = path_1.default.join(__dirname, '../../uploads/chunks_store.json');
...
function saveChunks(chunks) {
    const storedChunks = chunks.map((text, i) => ({
        id: `chunk-${i}`,
        text: text,
        keywords: tokenize(text),
    }));
    fs_1.default.writeFileSync(STORE_PATH, JSON.stringify(storedChunks));
...
function searchChunks(query, topK = 5) {
    ...
    const storedChunks = JSON.parse(fs_1.default.readFileSync(STORE_PATH, 'utf-8'));
